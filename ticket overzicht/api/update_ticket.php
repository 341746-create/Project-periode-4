<?php
// ============================================================
//  API: POST /ticket overzicht/api/update_ticket.php
...
try {
    // Haal reservering op
    $stmt = db()->prepare("SELECT * FROM reserveringen WHERE id = :id AND status NOT IN ('geannuleerd', 'voltooid')");
    ...
    $prijs_stmt = db()->prepare("SELECT prijs_per_stoel FROM voorstellingen WHERE id = :id");
